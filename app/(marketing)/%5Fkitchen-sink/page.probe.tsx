...
function AllPrimitives({ division }: { division: string }) {
  return (
    <>
      {/* Originally U-03 and Digital-only. `GS-P04` gave Design and Press per-service pages
          built from the same deliverables/exclusions/process structure, so the component was
          promoted to components/content/ and the specimen now renders in EVERY frame rather
          than only the digital one. That is the point of rendering it here: the row names no
          colour and no typeface, so what a reader has to be able to trust is that it stays
          legible under each theme's own tokens — and check-axe audits this route at two
          viewports in two phases, which is where that gets measured. */}
      <Specimen name="Data row — mono label, mono value, body rationale">
        <DataRows
