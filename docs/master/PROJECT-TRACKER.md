...
| Epic M | **`M-P1-7`** | **Sweep result for the `M-P1-6` class, and the remaining instance is the highest-stakes one.** `check-launch-content` reads `SANITY_DATASET` from the runner via `sanity/env.ts` and its own headline claims *"the statutory record is complete for the dataset being built"*. Against a deployment the dataset being built is the **deployment's**, which the runner cannot see — and on Vercel they now genuinely differ by design: Production → `production`, everything else → `development`. Pointed at a production deployment from a `development` runner it would report *"the live-only assertions do not apply"* and exit 0. **This is the gate that exists to stop a `[SEED]` VAT number reaching a public page**, so a premise it reads from the wrong machine is worse here than anywhere else it could occur. Remedy is the same as `M-P1-6`'s and Resend's — have the deployment report its own dataset and compare — and is **not applied**: it needs a route that reports it, which is a gate change of its own. **Also swept and clear:** `check-responsive` takes every fact from the browser it drives and has no environment premise; `check-axe`'s token probe derives its expectation from the served page, correctly. **Local by construction, and must not be pointed at a remote target without this being revisited:** `check-bundle-size` and `check:theme` both read `.next/` on disk, which is the runner's build and not the deployment's — today that is correct because they measure the artefact, but the same sentence describes how this class starts |
