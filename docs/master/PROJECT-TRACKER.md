...
| Epic M | `M-P2-22` | **`Footer` builds neither of the two link groups `APP-FLOW.md` §8 specifies — Company and Legal — and its docstring said the routes did not exist.** They do: `/about`, `/approach`, `/contact`, `/work`, `/insights` and `/legal/[slug]` all shipped in Epics N and L. The comment was written when it was true and was not revisited when the routes landed, which is the stale-spec class `CLAUDE.md` names; the comment is corrected in the same commit as this row, the missing markup is not. **Filed P2 but read the failure mode before leaving it there.** The consequence is that the only link to a legal document anywhere in the site's chrome is one on the Press landing page — so the privacy notice and cookie policy are reachable by URL and effectively by nothing else. `BEFORE-LAUNCH.md` does not cover it and no gate asserts it. That is a launch-shaped problem wearing a P2 label, on the same reasoning that promoted `M-P2-5` to `M-P1-2`. Sized at ~20 lines of markup plus the link list; the work is trivial and the priority is the open question |
