...
| 7 | A-06 | Sanity project + core schemas | P0 | 2d | — | **PART DONE**, UNBLOCKED | Dev | **`Q-M17` is resolved and this row's note was stale.** `M-05` delivered the project (`spzu6y31`), the two datasets, `sanity/env.ts`, `sanity.config.ts`, `sanity.cli.ts`, the shared `sanity/schemas/` folder, `lib/sanity/client.ts` and one document type — `companyDetails`. **What remains is the core schemas**: `project`, `service`, `legalDocument`, `teamMember` and the shared object types in `_shared/SCHEMA-CORE.md`. Nothing blocks them. **This is the next row.** One outstanding operator step for the Studio: the CORS origin, `SETUP.md` |
