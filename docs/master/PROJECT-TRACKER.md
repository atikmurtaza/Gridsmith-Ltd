...
| 6 | **A-GATE** | ⚑ **Epic A exit gate** | P0 | 0.5d | A-05a, A-10b | **DONE** 14 Aug | Dev | **PASSED — all six criteria.** The full record is in this file at § "A-GATE pass criteria" and in `_shared/05-HANDOVER.md` §2; seven audit rounds, the last four scoped, round 7 in `_shared/12-A-GATE-RUN-7.md`. **Do not reopen Epic A** — 22 P2 items are logged against it and none is an accessibility failure. ⚠ **This row read "OPEN — NEXT TASK / criterion 6 has never completed" until 2 September 2026**, nineteen days after the gate passed and while its own file said PASSED forty lines below. Epics M, N, L and S were all built in that window, so the row was also asserting that work which had already happened must not start. It is corrected here rather than deleted because **the stale row, not the gate, is the finding**: a tracker row is the first thing a fresh session reads, and this one would have sent it to re-run a completed audit |
...
| ID | Task | P | Est | Depends | Status | Owner | Notes |
|---|---|---|---|---|---|---|---|
| G-01 | Crawl existing site, export URLs | P0 | 0.5d | — | **DONE** | Dev | **Closed at `GS-R001`, 16 Sep 2026. It was never blocked and the site was never greenfield** — `LIVE-SITE-EXTRACT.md` established a live WordPress site on 7 Sep and this row went on saying otherwise. No crawl and no owner export was needed: the site publishes `wp-sitemap.xml`. **Eight URLs**, four of them WordPress/theme defaults. Inventory and proposed mapping: `LIVE-SITE-EXTRACT.md` §13 |
| G-02 | Build `redirects/legacy.json` | P0 | 1d | G-01 | **BLOCKED — on `GS-O007`, not on `G-01`** | Dev | File committed empty (`[]`) and wired into `next.config.ts`. Seven of the eight rows are settled by evidence; the eighth (`/terms-and-conditions/`) is an owner decision, because the live instrument is one document this build splits three ways. Populating it also needs a cutover, which `GS-R001` prohibits. `LIVE-SITE-EXTRACT.md` §13.2–§13.3 |
...
| G-04 | Sitemap, robots, `llms.txt` | P0 | 1d | N-* | **DONE (`llms.txt` deliberately not built)** | Dev | `GS-R001`. `app/robots.ts` and `app/sitemap.ts`, both reading one `INDEXABLE` from `lib/seo/site.ts` so they cannot disagree. **Default is `Disallow: /` and an empty sitemap**; indexing needs a Vercel production deployment AND an explicit `NEXT_PUBLIC_SITE_URL`, so it is an act rather than a side effect. Per-route canonicals via `alternates.canonical: './'` in all four root layouts, plus `metadataBase`, description, Open Graph and a `noindex` meta on non-production. **No `llms.txt`**: it is an unratified convention, and a file listing content for an AI crawler is a publication decision nobody has taken — raise it as an owner action if it is wanted |
| G-05 | Structured data pass | P0 | 1d | N-* | **DONE** | Dev | `GS-R001`. One `Organization` in the footer — which is the one component that already reads `companyDetails`, so no value has a second copy. **`brand[]` rather than `department[]`**: `department` asserts sub-organisations, and the divisions are trading names of one legal entity, which is the fact `/about` exists to stop people misreading. Not `LocalBusiness` — that wants a customer-facing premises and opening hours, and `GS-O004` publishes neither. No `logo`, no `sameAs`, no `aggregateRating` (`GS-O007`; and a rating computed from reviews rendered under Freelancer's terms is not Gridsmith's to publish as its own) |
...
| Epic M | `M-P1-3` **residual, found 4 Sept 2026 at the `K-10` premise check** | **The fix was right and three of its assertions were not measuring what they reported.** The route was re-checked by querying the live database as `anon` over HTTP and then breaking each branch deliberately. **(1) Two of the three `NO_READ` probes were INERT.** `sample_grants` and `events` hold no rows, so *"anon SELECT returned rows"* cannot fire for them however broken RLS is — a permissive `for select to anon using (true)` policy was added to each against the live database and the route stayed clean. The line `sample_grants: HTTP 200, 0 rows` was a reading of an empty table reported as a reading of row-level security: §22, live and shipped, inside the gate written for that class. Now labelled `NOT VALIDATED` and proven to fire once a row exists. **(2) `leads`’ validity was accidental** — it works only because the table happens to hold rows, which nothing asserted. A **read-back** was added: the probe row now carries a client-generated id and the route asks for that exact row back, so a 201 followed by an empty read is a row that demonstrably exists being filtered by the key that wrote it. **(3) There was no assertion that `anon` cannot INSERT into `sample_grants`**, which is `K-10`’s security premise: `token` is a bearer credential with a 72h expiry, so an insert policy there lets any browser mint a grant for any asset. Added for `sample_grants` and `events`, both proven by deliberate failure (HTTP 201 accepted → finding). **UPDATE and DELETE probes were written and REMOVED**: with a permissive `for update to anon using (true)` policy live, SQL as role `anon` affects 1 row and the same write through PostgREST affects 0 — PostgREST resolves a filtered write through a subselect, so with no SELECT policy it can never find a row to write. Over HTTP those branches cannot fire while the read-back is clean, which is unreachable code, not defence in depth (`A-GATE-4-3`). All five surviving branches proven red against the live database with rollback verified. **A second `check:rls:live` script was written and deleted before commit** — it duplicated this route by about 70%, which is what comes of writing before reading `app/api/`. |
...
| Epic M | ~~**`M-P2-22`**~~ | **FIXED 2 Sep, `7c069b0d`. Promoted out of P2 first, on the row's own reasoning.** The original: `Footer` built neither of the two link groups `APP-FLOW.md` §8 specifies, and its docstring said the routes did not exist. They did — `/about`, `/approach`, `/contact`, `/work`, `/insights` and `/legal/[slug]` all shipped in Epics N and L — so the comment was written when it was true and not revisited when the routes landed. The consequence was that **the only link to a legal document anywhere in the site's chrome was one on the Press landing page**, for eleven rounds: the privacy notice and the cookie policy were reachable by URL and effectively by nothing else. E-Commerce Regs reg. 6 requires the particulars *easily, directly and permanently accessible*, and a cookie policy reachable from the footer is the ICO's expectation. That is a launch-shaped problem wearing a P2 label, on the same reasoning that promoted `M-P2-5` to `M-P1-2`. **Why no gate fired is the part worth keeping.** `check-axe` already resolved every same-origin link on every audited route — but that assertion asks whether every link that *exists* points somewhere real, and **a missing link passes it by never being asked about**. The new assertion asks the opposite question: does a link that must exist exist at all, on every route rather than a sample, because "permanently" is not satisfied by a group that renders on the master route group and not on a division's. Expected paths are hardcoded, so deleting a slug does not delete the expectation with it. Proven by breaking each of three branches separately: a dropped slug (*13 of 14 routes* — not 14, because `/legal/privacy` still links `/legal/cookies` from its own other-documents list), a footer suppressed for `digital` only (*1 of 14*, naming `/digital`), and the hollow-subject branch pointed at a route that does carry the footer. The count moves in both directions and names different routes each time. `Careers` is the one §8 entry not shipped — there is no `/careers` route, and the resolve pass would fail the build, which is `nav.ts`'s stated policy. Cookie preferences is `ConsentReopen`, already permanent in the statutory block, and is not duplicated. Ledger row filed; `check:claims` needed an `M-P2-` round boundary, and that entry states its own weakness rather than implying a strength it has not got |
...
What must not happen — a draft presented as reviewed — is prevented by rendering the state, not by
hiding the document. **The `noindex` half was removed on 2 September 2026** (`F-15`): it was the
one place `solicitorApproved` acted as a gate rather than a description, and it withheld the
instruments the company actually publishes from search pending a review that was not booked.
