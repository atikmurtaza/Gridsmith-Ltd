...
| Epic M | **`M-P1-5`** | **CI has not run since 13 Aug. Every push on 20 Aug failed at workflow STARTUP — zero jobs, 0s duration, "this run likely failed because of a workflow file issue".** The YAML parses and Actions is enabled (`allowed_actions: all`), so the cause is upstream of the file — most likely private-repo Actions minutes, which needs a scope this session does not hold to confirm. **"CI is the arbiter" (`CLAUDE.md`) is currently false**, and has been for every commit in the last three sessions. `check:control` was wired into `ci.yml` and has therefore *never executed there*; the same is true of every gate added since. This is `P1` because it is not one broken gate, it is the absence of the arbiter — and it announced itself only because a deploy made someone look at the Actions tab |
| Epic M | **`M-P1-6`** | **`check-axe`'s analytics premise reads the RUNNER's environment while asserting the SERVER's behaviour — the second instance of the class `CLAUDE.md` already documents for Resend, and the first one found by running against a deployment.** Run against the Vercel preview without `.env.local`, the gate reported *"no analytics id is configured, so the grant path cannot be exercised"* about a server that had `NEXT_PUBLIC_GA4_ID` and `NEXT_PUBLIC_POSTHOG_KEY` set. On `localhost` the two environments are the same process, which is why it has always looked correct. The Resend instance was fixed by having the probe route report its own configuration and comparing against that; the same remedy applies here and is **not applied** — it is a gate change, not a deploy change |
