...
| Epic M | `M-P2-37` | **`public/` is excluded from `check-service-role-key`, and it is the most dangerous tree in the repository for a leaked key** — everything in it is served verbatim. The recorded reason, *"served verbatim, never bundled"*, is the argument for scanning it restated as the argument against. Clean today for two accidents: the tree holds one hand-written file, and the gate's extension set is `ts\|tsx\|js\|jsx\|mjs\|cjs`, so a `.html` would not match even if scanned. **Add one `.js` to `public/` and the gap is live.** Not fixed in the sweep because it means changing what that gate considers source. The load-bearing `.next/static/chunks` sweep is unaffected |
