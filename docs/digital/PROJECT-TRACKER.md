...
| Item | Outcome |
|---|---|
| Capability groups | Web · Software · Apps & Interactive · Automation & Intelligence · Operate & Improve (`_shared/SERVICE-ARCHITECTURE.md`) |
| `U-08` service page | **Reconciled.** No price, no `notFound()` on a missing price; renders summary, description, deliverables/exclusions, process, collaborators, related services; CTA **Discuss Your Project** → `/contact?division=digital&service=<slug>`, secondary **Contact Gridsmith** |
| `V-06` `/digital/estimate` | **REMOVED** — it only published indicative bands. `V-07`–`V-16`, `Y-08`, `Y-09`, `Q-DG1`, `Q-DG2` stay superseded. A non-price Project Scoper is an optional later enhancement, not a launch row |
...
| Paid media | **RESOLVED at `GS-P06`** (`GS-O013`). Paid media management and placement within managed advertising accounts is a Master engagement row (18 → 19 activities). The catalogue is unchanged at 81 approved services and no Digital service claims it |
| **Ownership and handover copy** | **CORRECTED at `GS-P06`** (`GS-O013`). *"Hosting, domain and CMS accounts are yours"* and *"Code, infrastructure and accounts are yours"* were absolute contractual statements in marketing copy. They now read: ownership, source-code handover, account access and infrastructure arrangements are **defined in the written project agreement**, with Gridsmith's preference for client-controlled arrangements stated as a preference. No legal term was drafted and no existing legal gate was overridden |
| **Hosting** | **`"Gridsmith does not resell hosting"` STRUCK at `GS-P06`** (`GS-O013`) — a permanent rule foreclosing managed-hosting arrangements the owner has not ruled out. `maintenance-and-monitoring` is now *Maintenance, Hosting Coordination & Monitoring*: coordination, configuration, deployment, maintenance, monitoring and management, arranged per project. **No hosting product, SLA or price was invented**; the approved capability name `Hosting coordination` is untouched |
| **Accessibility claim** | **`"No one can certify accessibility"` STRUCK at `GS-P06`** — categorical and false. Formal certification is **not included unless explicitly scoped**, and Gridsmith reports the standards tested, the evidence, the findings and the residual issues. WCAG 2.2 AA positioning, the automated/manual distinction and the residual-risk language are all kept |
| **Ranking guarantees** | Reworded from *"Not offered by anyone honestly"* to *"Gridsmith does not guarantee search rankings or traffic outcomes"*. The limitation is unchanged; the swipe at the market is gone |
| **Digital review block** | **REMOVED at `GS-P06`** (`GS-O014` amendment) — Freelancer reviews are Master-only |
| **Process — Delivery and Support** | Handover covers the running system, written deployment documentation, and the source-code and account arrangements **the written agreement sets out**. Maintenance and further development are scoped as further work or an ongoing engagement |
...
| U-03 | **Data row component** | P0 | 1d | U-01 | **DONE** 2 Sep · **MOVED** 16 Sep | Dev | **Now `components/content/DataRows.tsx`.** Promoted out of `components/divisions/digital/` at `GS-P04` on exactly the condition this row and the component docstring both set — *sharing is a guess until a second division asks*. Two did: Design and Press gained per-service pages built from the same deliverables/exclusions/process structure, and the alternative was a Design page importing from `divisions/digital/`. It did **not** go to `primitives/`: it is a composed content pattern, not a theme-agnostic building block. It names no colour and no typeface, so each theme resolves its own `--font-mono`, `--line` and `--ink-muted` — and the mono label is not a Digital mannerism, it is `CLAUDE.md`s one cross-division convention that *monospace marks anything verifiable*. The kitchen-sink specimen now renders in **every** theme frame rather than only the digital one, which is what a shared component actually needs audited. `components/divisions/digital/` is empty and deleted. Original entry: `components/divisions/digital/DataRow.tsx` — a `<dl>`, mono label · mono value · body prose, zero client JS. **Not in `primitives/`**: `DESIGN.md` names four consumers and all four are Digital, so sharing is a guess until a second division asks. Specimen committed on `/_kitchen-sink` inside the digital frame, so `check-axe` reaches it — 60 analyses, zero violations. The three-cue rule it sits under now has a gate, `check:state-cues`, which **found a live colour-alone state on its first run** (`.breadcrumbCurrent`) |
