 ...
 * is not selectable from outside the build; the attribute is the stable handle.
 *
 * **Zero subjects is a hard failure, not a pass.** The route list below includes the landing,
 * contact, withheld Path Finder and audiobook service. The measure assertion is per-block, so
 * a route with no `Prose` contributes nothing to it and is audited by the body assertions alone.
 *
 * ## The ceiling
 *
 ...
 *
 * GS-PRESS-001-R3 retired the old rights-block margin note. This gate measures the body and
 * rendered prose that remain; the former P-03 branch has no public subject and is retired.
 *
...
const ROUTES = ['/press', '/press/contact', '/press/path-finder', '/press/services/audiobook-production-support'];
...
if (measureChecks === 0) {
  console.error(
    '\ncheck-press-type: no [data-prose] block was found on any Press route, so the 52ch measure' +
      ...
      '\nThe Press landing and service pages should carry it. Restore it or delete this' +
